<x-layout>
    <div class="max-w-sm mx-auto bg-gray-900 border border-gray-800 p-8 rounded-lg mt-12">
        <h2 class="text-2xl font-bold mb-6 text-center text-gray-100 font-mono">Register</h2>
        ...
        <form method="POST" action="/register">
            @csrf
            <input type="text" name="name" placeholder="Name" required class="bg-gray-800 text-gray-100 placeholder-gray-500 border border-gray-700 rounded w-full px-3 py-2 mb-3 focus:outline-none focus:border-green-500">
            ...
            <button type="submit" class="bg-green-600 text-white w-full py-2 rounded hover:bg-green-500 font-medium transition">Register</button>
        </form>

        <p class="text-center text-sm mt-4 text-gray-400">
            Already have an account? <a href="/login" class="text-green-400 hover:underline">Login</a>
        </p>
    </div>

    <div class="fixed inset-0 pointer-events-none -z-10 opacity-30 overflow-hidden" aria-hidden="true">
        <canvas id="registerBgCandleChart" class="w-full h-full" style="filter: drop-shadow(0 0 10px rgba(6, 182, 212, 0.45)) drop-shadow(0 0 18px rgba(99, 102, 241, 0.35));"></canvas>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const ctx = document.getElementById('registerBgCandleChart');
            if (!ctx) return;
